...
import { createGraphQLError } from 'src/utils/create-graphql-error';
...
export const CurrencyScalar = new GraphQLScalarType({
  name: 'CurrencyCode',
  description: `A field whose value is a Currency: ${specifiedByURL}.`,
  serialize(value) {
  ...
  parseLiteral(ast) {
    if (ast.kind !== Kind.STRING) {
      throw createGraphQLError(
        `Can only validate strings as a currency code but got a: ${ast.kind}`,
        {
          nodes: ast,
        },
      );
